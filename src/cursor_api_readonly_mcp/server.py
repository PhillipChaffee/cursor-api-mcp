"""Read-only MCP tools wrapping Cursor Cloud Agents and Admin APIs."""
...
mcp = MCPServer(
    "cursor-api-readonly",
    instructions=(
        "Read-only access to Cursor HTTP APIs. Tools only fetch data — they cannot "
        "create, cancel, archive, or mutate agents, members, spend limits, or settings. "
        "Cloud Agents tools work with a user API key on all plans. Team Admin / Org "
        "tools require Enterprise keys with the appropriate scopes."
    ),
)


def _client() -> CursorApiClient:
...
def list_repositories() -> dict[str, Any]:
    ...
    """
    try:
        return _client().get("/v1/repositories")
    except Exception as exc:
        return _error_payload(exc)
...
def get_agent(agent_id: str) -> dict[str, Any]:
    ...
    """
    try:
        return _client().get(f"/v1/agents/{agent_id}")
    except Exception as exc:
        return _error_payload(exc)
...
def get_agent_run(agent_id: str, run_id: str) -> dict[str, Any]:
    ...
    """
    try:
        return _client().get(f"/v1/agents/{agent_id}/runs/{run_id}")
    except Exception as exc:
        return _error_payload(exc)
...
def list_agent_artifacts(agent_id: str) -> dict[str, Any]:
    ...
    """
    try:
        return _client().get(f"/v1/agents/{agent_id}/artifacts")
    except Exception as exc:
        return _error_payload(exc)
...
def list_team_members() -> dict[str, Any]:
    ...
    try:
        return _client().get("/teams/members")
    except Exception as exc:
        return _error_payload(exc)
...
def get_daily_usage_data(
    ...
    """Fetch daily team usage metrics (POST /teams/daily-usage-data; read-only).
    ...
    try:
        return _client().post_json("/teams/daily-usage-data", body=body)
    except Exception as exc:
        return _error_payload(exc)
...
def get_spending_data(
    ...
    """Fetch current-cycle team spend (POST /teams/spend; read-only).
    ...
    if page_size is not None:
        body["pageSize"] = page_size
    try:
        return _client().post_json("/teams/spend", body=body)
    except Exception as exc:
        return _error_payload(exc)
...
def get_usage_events(
    ...
    """Fetch filtered usage events (POST /teams/filtered-usage-events; read-only).
    ...
    if users is not None:
        body["users"] = users
    try:
        return _client().post_json("/teams/filtered-usage-events", body=body)
    except Exception as exc:
        return _error_payload(exc)
...
def list_team_repo_blocklists() -> dict[str, Any]:
    ...
    try:
        return _client().get("/settings/repo-blocklists/repos")
    except Exception as exc:
        return _error_payload(exc)
